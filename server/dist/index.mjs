import { a as message, c as and, d as gte, f as inArray, i as chat, l as eq, m as sql, n as isDatabaseAvailable, o as asc, p as lt, r as drizzle, s as desc, u as gt } from "./connection-pool-Ccv9srHa.mjs";
import "./src-BaHhVWSg.mjs";
import { n as getAuthMethodDescription, r as getAuthSession, t as getAuthMethod } from "./src-CKoYY1O3.mjs";
import { t as src_default$1 } from "./src-D1zibvPP.mjs";
import { i as extractApprovalStatus, o as DATABRICKS_TOOL_CALL_ID, s as DATABRICKS_TOOL_DEFINITION } from "./mcp-approval-utils-DEd3wq6H.mjs";
...
async function ensureDb() {
	...
	if (authMethod === "oauth" || authMethod === "cli") {
		...
		try {
			const { getDb } = await import("./connection-pool-CjRraShz.mjs");
...
async function getServerProvider() {
	const { getDatabricksServerProvider } = await import("./src-CWtouqVa.mjs");
	return getDatabricksServerProvider();
}
let cachedServerProvider = null;
const myProvider = { async languageModel(id, session) {
...
chatRouter.post("/", requireAuth, async (req, res) => {
	...
	try {
		...
		const result = streamText({
			model: await myProvider.languageModel(selectedChatModel, session),
...
async function generateTitleFromUserMessage({ message: message$1, session }) {
	const { text: title } = await generateText({
		model: await myProvider.languageModel("title-model", session),
		system: `\n
