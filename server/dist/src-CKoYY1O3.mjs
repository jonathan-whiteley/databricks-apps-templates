...
*/
async function getDatabricksTokenForRequest(session) {
	...
	try {
		...
		if (isTestEnvironment) {
			...
			return {
				...
				accessToken: oboAccessToken || void 0
			};
		}
		if (getRequestHeader("X-Forwarded-User")) {
			console.log("[getAuthSession] Using Databricks Apps headers");
			if (oboAccessToken) console.log("[getAuthSession] OBO authentication enabled - using user access token");
			else console.log("[getAuthSession] No OBO token found - will fall back to service principal");
...
//#endregion
export { getDatabaseUsername as a, getDatabricksUserIdentity as c, getCachedCliHost as i, getAuthMethodDescription as n, getDatabricksToken as o, getAuthSession as r, getDatabricksTokenForRequest as s, getAuthMethod as t };